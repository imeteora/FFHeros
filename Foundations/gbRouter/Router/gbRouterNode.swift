...
class gbRouterNode
{
    public typealias RouterNodeResultType = (gbRouterNode, [String : String])
    ...
    var ruby: Any? = nil
    weak var parentNode: gbRouterNode? = nil
    var childNotes: [gbRouterNode] = []
    ...
    func mappingKeyValuesTree(_ object: Any!, withKeyPath keyPath: [String]!)
    {
        ...
        var node: gbRouterNode? = self.childNode(with: key)
        if node == nil {
            node = gbRouterNode()
            node?.keyPath = key
    ...
    /// - Returns: 对应键的值（子节点）
    func childNode(with keyPath: String!) -> gbRouterNode? {
        var result: gbRouterNode? = nil
    ...
    func recursiveFindChildNode(_ url: String!) -> RouterNodeResultType?
    {
        ...
        let subNode: gbRouterNode? = self.childNode(with: key)
